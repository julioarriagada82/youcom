...
namespace YouCom.bll
{
    public class BannerBLL
    {
        ...
        public static IList<BannerDTO> getListadoBannerByCondominioByComunidad(decimal idCondominio, decimal idComunidad)
        {
            var banner = listaBannerActivo().Where(x => x.TheCondominioDTO.IdCondominio == idCondominio && x.TheComunidadDTO.IdComunidad == idComunidad).ToList();
            return banner;
        }
