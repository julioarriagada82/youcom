...
namespace YouCom.bll
{
    public class ComercioBLL
    {
        ...
        public static IList<ComercioDTO> getListadoComercioByCondominioByComunidad(decimal idCondominio, decimal idComunidad)
        {
            var comercios = listaComercioActivo().Where(x => x.TheCondominioDTO.IdCondominio == idCondominio && x.TheComunidadDTO.IdComunidad == idComunidad).ToList();
            return comercios;
        }
        ...
        {
            bool respuesta = ComercioDAL.ActivaComercio(theComercioDTO);
            return respuesta;
        }
