...
namespace YouCom.bll
{
    public class EventoBLL
    {
        ...
        public static IList<EventoDTO> getListadoEventoByCondominioByComunidad(decimal idCondominio, decimal idComunidad)
        {
            var eventos = listaEventoActivo().Where(x => x.TheCondominioDTO.IdCondominio == idCondominio && x.TheComunidadDTO.IdComunidad == idComunidad).ToList();
            return eventos;
        }
