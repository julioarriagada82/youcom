...
public partial class Privado_Mercado_MisVentas : Intermedia.IMSystem.Navigation.Page.WebPage
{
    ...
    protected void cargarVentas()
    {
        ...
        if (myAvisosDTO.Any())
        {
            rptVentas.DataSource = myAvisosDTO.Where(x => x.TheFamiliaDTO.RutFamilia == myUsuario.Rut).ToList();
            rptVentas.DataBind();
        }
    }
    ...
    protected void rptVentas_ItemDataBound(object sender, RepeaterItemEventArgs e)
    {
        try
        {
            ...
            {
                if (!string.IsNullOrEmpty(((YouCom.DTO.Avisos.AvisoDTO)e.Item.DataItem).IdAviso.ToString()))
                {
                    Literal myLitFechaCompra = new Literal();
                    Literal myLitRutComprador = new Literal();

                    myLitFechaCompra = (Literal)e.Item.FindControl("LitFechaCompra");
                    myLitRutComprador = (Literal)e.Item.FindControl("LitRutComprador");

                    myLitRutComprador.Text = !string.IsNullOrEmpty(((YouCom.DTO.Avisos.AvisoDTO)e.Item.DataItem).RutComprador) ? YouCom.Service.Generales.Formato.FormatoRut(((YouCom.DTO.Avisos.AvisoDTO)e.Item.DataItem).RutComprador) : string.Empty;
                    myLitFechaCompra.Text = Convert.ToDateTime(((YouCom.DTO.Avisos.AvisoDTO)e.Item.DataItem).FechaCompra) != DateTime.MinValue ? Convert.ToDateTime(((YouCom.DTO.Avisos.AvisoDTO)e.Item.DataItem).FechaCompra).ToShortDateString() : string.Empty;
